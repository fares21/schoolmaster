...
class AIOrchestrator
{
    ...
    public function processRequest(string $prompt, int $userId, int $schoolId): array
    {
        ...
        $estimatedTokens = $this->tokenCounter->countTokens($prompt) * 1.05; // 5% safety margin
        ...
        // Check budget using atomic operation to prevent race conditions
    ...
    private function callDeepSeek(string $prompt): string
    {
        ...
        $response = Http::withHeaders([
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => 'أنت مساعد ذكي متخصص في مجال التعليم وإدارة المدارس. أجب باللغة العربية الفصحى. كن دقيقاً ومفيداً. تجنب الإطالة غير الضرورية. ركز على الحلول العملية.',
                ],
                [
                    'role' => 'user',
                    'content' => $prompt,
    ...
    private function getRemainingBudget(int $schoolId, string $budgetKey): float
    {
        ...
        $used = (float) Cache::get($budgetKey, 0);
        
        // If not in cache, calculate from database
        if ($used === 0) {
            $used = AIUsage::where('school_id', $schoolId)
    ...
    {
        Cache::increment($budgetKey, $cost);
    }
    ...
    private function sendLowBudgetAlert(int $schoolId, float $remaining): void
    {
        ...
        // TODO: Send email/slack notification to super admin
    }
}